...
## Decision
- DB transactions are started and committed in application services where multi-step consistency is required.
- Infrastructure repositories receive a `&rusqlite::Transaction` parameter and execute within it.
- `Database::with_transaction` (`src-tauri/src/db/mod.rs`) is the standard entry point for transactional work.
- SQL strings and `rusqlite` usage are restricted to `infrastructure/` and `src-tauri/src/db/` modules.

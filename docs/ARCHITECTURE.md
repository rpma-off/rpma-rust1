...
- **Sync queue : statut sérialisé en JSON** mais filtré en SQL brut (`status = 'pending'` dans `sync/queue.rs`). Le code d’enqueue utilise `serde_json::to_string`, donc la colonne `status` stocke une chaîne JSON (ex: `"pending"` avec guillemets), ce qui ne matche pas la comparaison brute.
...
5. **Hasher les tokens au repos**
   ...
   let app_secret = config.hmac_secret(); // recommandé ≥ 32 octets pour la sécurité

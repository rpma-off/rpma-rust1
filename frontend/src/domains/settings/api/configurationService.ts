...
export const configurationService = {
  ...
  async getSystemConfigurationById(
    id: string,
  ): Promise<ServiceResponse<Configuration>> {
  ...
  async updateSystemConfiguration(
    ...
    try {
      const nextValue = config["value"] as JsonValue | undefined;
      await ipcClient.settings.updateGeneralSettings(
        nextValue === undefined ? {} : ({ [id]: nextValue } as JsonObject),
      );
  ...
  async getBusinessHoursConfig(): Promise<ServiceResponse<JsonObject>> {
    try {
      const settings =
        (await ipcClient.settings.getAppSettings()) as AppSettings | null;
  ...
  async updateBusinessHoursConfig(
    hours: JsonObject,
  ): Promise<ServiceResponse<JsonObject>> {
    try {
  ...
  async getBusinessRules(
    _filters?: unknown,
  ): Promise<ServiceResponse<BusinessRule[]>> {
    try {

...
describe('useTaskMutations — auth guard (requireToken)', () => {
  const ERR = 'Utilisateur non authentifié';

  it('updateTask throws when no session', async () => {
    const { updateTask } = renderMutations(undefined);
    await expect(
      (updateTask as unknown as { mutateAsync: (...args: unknown[]) => Promise<unknown> }).mutateAsync({ taskId: 't1', data: {} })
    ).rejects.toThrow(ERR);
  });
  ...
  it('deleteTask throws when no session', async () => {
    ...
    await expect(
      (deleteTask as unknown as { mutateAsync: (...args: unknown[]) => Promise<unknown> }).mutateAsync('t1')
    ).rejects.toThrow(ERR);
  });
  ...
  it('editTask throws when no session', async () => {
    ...
    await expect(
      (editTask as unknown as { mutateAsync: (...args: unknown[]) => Promise<unknown> }).mutateAsync({ taskId: 't1', data: {} })
    ).rejects.toThrow(ERR);
  });
  ...
  it('reportIssue throws when no session', async () => {
    ...
    await expect(
      (reportIssue as unknown as { mutateAsync: (...args: unknown[]) => Promise<unknown> }).mutateAsync({
        taskId: 't1', issueType: 'mechanical', severity: 'high', description: 'Broken',
      })
    ).rejects.toThrow(ERR);
  });

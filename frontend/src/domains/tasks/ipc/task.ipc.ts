...
type TaskListFilters = Partial<TaskQuery> & {
  page?: number | null;
  limit?: number | null;
};

const buildTaskQuery = (filters: TaskListFilters): TaskQuery => ({
  pagination: {
    page: filters.page ?? 1,
...
export const taskIpc = {
  ...
  get: async (id: string): Promise<Task | null> => {
    return cachedInvoke(
      `task:${id}`,
      IPC_COMMANDS.TASK_GET,
      {
        id,
      },
      (data: JsonValue) =>
        extractAndValidate(data, validateTask, {
          handleNotFound: true,
        }) as Task | null,
    );
  },
  ...
  list: async (filters: TaskListFilters): Promise<TaskListResponse> => {
    const result = await safeInvoke<JsonValue>(IPC_COMMANDS.TASK_LIST, {
      filters: buildTaskQuery(filters),
    });
  ...
  checkTaskAssignment: async (
    taskId: string,
    userId: string,
  ): Promise<TaskAssignmentCheckResponse> => {
    return safeInvoke<TaskAssignmentCheckResponse>(
      IPC_COMMANDS.CHECK_TASK_ASSIGNMENT,
      {
  ...
  checkTaskAvailability: async (
    taskId: string,
  ): Promise<TaskAvailabilityCheckResponse> => {
    return safeInvoke<TaskAvailabilityCheckResponse>(
      IPC_COMMANDS.CHECK_TASK_AVAILABILITY,
      {
    ...
    return safeInvoke<string>(IPC_COMMANDS.EXPORT_TASKS_CSV, {
      request: {
        ...
        filter: options.date_range
          ? {
              date_from: options.date_range.start_date ?? null,
              date_to: options.date_range.end_date ?? null,
            }
          : null,
      },
    });
  ...
  draftSave: async (formData: string): Promise<void> => {
    await safeInvoke<void>(IPC_COMMANDS.TASK_DRAFT_SAVE, {

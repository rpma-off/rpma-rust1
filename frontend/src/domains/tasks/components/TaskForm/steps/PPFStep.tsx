...
export const PPFStep: React.FC<FormStepProps> = ({
  ...
  const getZoneCategoryColor = (category: string) => {
    const colors: Record<string, string> = {
      front: "bg-[hsl(var(--rpma-teal))]/20 text-[hsl(var(--rpma-teal))] border-[hsl(var(--rpma-teal))]/30",
      rear: "bg-red-500/20 text-red-400 border-red-500/30",
      side: "bg-[hsl(var(--rpma-teal))]/20 text-[hsl(var(--rpma-teal))] border-[hsl(var(--rpma-teal))]/30",
      roof: "bg-purple-500/20 text-purple-400 border-purple-500/30",
      full: "bg-[hsl(var(--rpma-teal))]/20 text-[hsl(var(--rpma-teal))] border-[hsl(var(--rpma-teal))]/30",
    };
    return (
      colors[category] ||
  ...
  return (
    <div className="p-3 sm:p-6 max-w-4xl mx-auto">
      <div className="bg-muted rounded-lg border border-border overflow-hidden">
        {/* Header */}
        <div className="bg-gradient-to-r from-[hsl(var(--rpma-teal))]/10 to-[hsl(var(--rpma-teal))]/5 p-3 sm:p-4 border-b border-border">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
            <div className="flex items-center">
              <Shield className="w-5 h-5 text-[hsl(var(--rpma-teal))] mr-2" />
              <h3 className="text-base sm:text-lg font-semibold text-foreground">
                Zones de protection PPF
              </h3>
            </div>
            <div className="text-xs sm:text-sm bg-[hsl(var(--rpma-teal))]/20 px-2 sm:px-3 py-1 rounded-full">
            ...
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-3 py-2 border border-border bg-white text-foreground placeholder-muted-foreground rounded-lg focus:border-[hsl(var(--rpma-teal))] focus:ring-2 focus:ring-[hsl(var(--rpma-teal))]/20 focus:outline-none"
                />
              </div>
            </div>
                  ...
                  className={`
                    ...
                    ${
                      selectedCategory === category.id
                        ? "bg-[hsl(var(--rpma-teal))] text-foreground border-[hsl(var(--rpma-teal))]"
                        : "bg-white text-foreground border-border hover:bg-[hsl(var(--rpma-surface))]"
                    }
                  `}
                >
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[hsl(var(--rpma-teal))]/20
                    ${
                      isSelected
                        ? "border-[hsl(var(--rpma-teal))] bg-white text-foreground"
                        : "border-border bg-white text-foreground hover:border-[hsl(var(--rpma-teal))]/40 hover:bg-[hsl(var(--rpma-surface))]"
                    }
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    {isSelected && (
                      <CheckCircle className="w-5 h-5 text-[hsl(var(--rpma-teal))] flex-shrink-0 ml-2" />
                    )}
                  </div>
          ...
          <div className="border-t border-border pt-6">
            ...
            <div className="flex gap-2 mb-4">
              <input
                ...
                className="flex-1 px-3 py-2 border border-border bg-white text-foreground placeholder-muted-foreground rounded-lg focus:border-[hsl(var(--rpma-teal))] focus:ring-2 focus:ring-[hsl(var(--rpma-teal))]/20 focus:outline-none"
                disabled={isLoading}
              />
              <button
                type="button"
                ...
                className="px-4 py-2 bg-[hsl(var(--rpma-teal))] text-foreground rounded-lg hover:bg-[hsl(var(--rpma-teal))]/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
              ...
              formData.custom_ppf_zones.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {formData.custom_ppf_zones.map((zone, index) => (
                    <div
                      key={index}
                      className="flex items-center bg-[hsl(var(--rpma-teal))]/20 text-[hsl(var(--rpma-teal))] px-3 py-1 rounded-full text-sm border border-[hsl(var(--rpma-teal))]/30"
                    >
                      <span>{zone}</span>
                      <button
                        type="button"
                        ...
                        disabled={isLoading}
                        className="ml-2 text-[hsl(var(--rpma-teal))] hover:text-foreground disabled:opacity-50"
                      >
                        <X className="w-3 h-3" />
                      </button>
                    </div>
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-3 py-2 border border-border bg-white text-foreground placeholder-muted-foreground rounded-lg focus:border-[hsl(var(--rpma-teal))] focus:ring-2 focus:ring-[hsl(var(--rpma-teal))]/20 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
                  disabled={isLoading}
              ...
              >
                ...
                {formData.lot_film && (
                  <div className="bg-[hsl(var(--rpma-teal))]/10 border border-[hsl(var(--rpma-teal))]/30 rounded p-2">
                    <p className="text-[hsl(var(--rpma-teal))] font-medium">
                      Format détecté: {formData.lot_film}
                    </p>
                    <p className="text-[hsl(var(--rpma-teal))] text-xs">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {formData.ppf_zones.map((zoneId) => {
                        ...
                        return zone ? (
                          <span
                            key={zoneId}
                            className="text-xs bg-[hsl(var(--rpma-teal))]/20 text-[hsl(var(--rpma-teal))] px-2 py-1 rounded border border-[hsl(var(--rpma-teal))]/30"
                          >
                            {zone.name}
                          </span>
                        ) : null;

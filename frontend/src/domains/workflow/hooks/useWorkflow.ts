...
import { WorkflowService } from '../server';
...
export const useWorkflow = (options: UseWorkflowOptions = {}) => {
  ...
  const startWorkflow = useCallback(async (data: CreateWorkflowExecutionDTO) => {
    try {
      ...
      setError(null);
       ...
       toast.success('Flux de travail d�marr� avec succ�s');
       return workflow;
    } catch (err: unknown) {
      const normalizedError = normalizeError(err);
  ...
  const completeStep = useCallback(async (data: CompleteStepDTO) => {
    ...
    try {
      ...
      toast.success('�tape termin�e avec succ�s');
      return result;
    } catch (err: unknown) {
      const normalizedError = normalizeError(err);
  ...
  const startStepTiming = useCallback(async (data: StartTimingDTO) => {
    try {
      ...
      toast.success('Minuterie d�marr�e');
      return timing;
    } catch (err) {
  ...
  const uploadStepPhotos = useCallback(async (
    ...
    try {
      ...
      toast.success(`${urls.length} photo(s) t�l�vers�e(s) avec succ�s`);
      return urls;
    } catch (err) {
  ...
  }, [workflow, currentStep, workflowService, user?.id, user?.token]);

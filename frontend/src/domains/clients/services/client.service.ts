...
function isLegacyStatisticsResponse<T>(
  value: unknown,
): value is LegacyStatisticsResponse<T> {
  return (
    typeof value === "object" &&
    value !== null &&
    "type" in value &&
...
const buildUpdateRequest = (
  ...
  customer_type: (body.customer_type ??
    null) as UpdateClientRequest["customer_type"],
...
export class ClientService {
  static async getClientWithTasks(
    id: string,
    sessionToken?: string,
  ): Promise<ApiResponse<ClientWithTasks>> {
    if (!sessionToken) {
      return {
        success: false,
        error: new ApiError(
    ...
    try {
      ...
      if (!validation.success) {
        return {
          success: false,
          error:
            validation.error.issues[0]?.message ?? "Invalid client payload",
          status: 400,
        };
      }
  ...
  static async getClientsWithTasks(
    sessionToken?: string,
    query?: {
      page?: number;
      limit?: number;
    ...
    ApiResponse<{ data: ClientWithTasks[]; pagination: PaginationInfo }>
  > {
    if (!sessionToken) {
      return {
        success: false,
    ...
    try {
      ...
      try {
        const clients = await ipcClient.clients.listWithTasks(
          clientQuery,
          limitTasks || 5,
        );
        // Keep the in-memory shape compatible with PaginationInfo while avoiding
        // BigInt values in JSON responses from API routes that serialize this result.
        const page = query?.page || 1;
        const limit = query?.limit || 20;
        const total = BigInt(clients.length);
        ...
        const clientsWithTasks: ClientWithTasks[] = listResponse.data.map(
          (client: Client) => ({
            ...client,
            tasks: [],
          }),
      ...
      return {
        ...
        error: new ApiError(
          error instanceof Error
            ? error.message
            : "Failed to fetch clients with tasks",
          500,
          "CLIENTS_FETCH_FAILED",
        ),
        data: undefined,

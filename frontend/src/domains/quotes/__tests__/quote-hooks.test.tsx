...
import { useDuplicateQuote, useQuoteExportPdf } from '../hooks/useQuoteOperations';
...
describe('quote hooks IPC contract handling', () => {
  ...
  it('loads attachments from unwrapped safeInvoke payloads', async () => {
    ...
    await waitFor(() => expect(result.current.attachments).toEqual(attachments));
  });
  ...
  it('returns unwrapped quote operations payloads', async () => {
    ...
    await expect(duplicateHook.result.current.duplicateQuote('quote-1')).resolves.toEqual(duplicatedQuote);
    await expect(exportHook.result.current.exportPdf('quote-1')).resolves.toEqual(exportResponse);
  });
});

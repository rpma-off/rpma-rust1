...
export async function GET(
  ...
  try {
    ...
    console.error(
      "Error in GET /api/admin/configuration/category/[category]:",
      error,
    );
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    ...
    for (const config of configurations) {
      if (config.id) {
        const result = (await configurationService.updateSystemConfiguration(
          config.id,
          config,
        )) as ServiceResponse<SystemConfiguration>;
        results.push(result);
      }
    }
    ...
    console.error(
      "Error in PUT /api/admin/configuration/category/[category]:",
      error,
    );
    return NextResponse.json(
      { error: "Internal server error" },

...
export default function EditClientPage({ params }: EditClientPageProps) {
  ...
  const loadClient = useCallback(async () => {
    ...
    try {
      ...
      if (response.error) {
        setError(typeof response.error === 'string' ? response.error : response.error.message || t('clients.clientNotFound'));
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!params?.id || !user) {
      toast.error(t('clients.invalidRequest'));
      return;
    }
    ...
    try {
      ...
      if (!user?.token) {
        setFormErrors({ general: t('clients.authenticationRequired') });
        return;
      }

      const response = await clientService.updateClient(params.id, formData, user.token);
      if (response.error) {
        setFormErrors({ general: response.error || t('clients.failedToUpdate') });
        return;
      }
      ...
      setFormErrors({ general: t('clients.unexpectedError') });
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <Card className="border-red-700/50 bg-red-900/20">
          <CardContent className="pt-6">
            <div className="text-center">
              <p className="text-red-400 text-lg font-medium">{error || t('clients.clientNotFound')}</p>
              <p className="text-muted-foreground text-sm mt-2">{t('clients.checkClientId')}</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="rpma-shell p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-6">
            ...
            >
              <ArrowLeft className="h-4 w-4" />
              <span className="text-sm">{t('clients.backToClient', { name: client.name })}</span>
            </Link>
            <div className="flex items-center space-x-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-3">
              <label className="block text-sm font-medium text-muted-foreground">
                {t('clients.customerTypeRequired')}
              </label>
            ...
            <div className="space-y-6">
              <h3 className="text-lg font-semibold text-foreground border-b border-[hsl(var(--rpma-border))] pb-2">{t('clients.addressInformation')}</h3>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder={t('clients.enterAdditionalNotes')}
                rows={4}
              />

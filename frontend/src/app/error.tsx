      ...
      <div className="max-w-2xl w-full mx-4">
        ...
        <div className="rpma-shell p-8 md:p-12">
          <div className="text-center mb-8">
            ...
            <p className="text-base md:text-lg text-border-light mb-6 leading-relaxed">
              Une erreur inattendue s&apos;est produite. Notre équipe a été notifiée et travaille à résoudre le problème.
            </p>

            {/* Error Details for Development */}
            {isDev && error.stack && (
              <div className="mt-6 p-4 bg-background/50 rounded-xl border border-border/30 text-left">
                <details className="group">
                  <summary className="text-sm font-medium text-border-light cursor-pointer hover:text-foreground transition-colors flex items-center gap-2">
                  ...
                  <div className="mt-3 space-y-3">
                    <pre className="text-xs text-red-400 whitespace-pre-wrap bg-background/50 p-3 rounded-lg border border-red-500/20 overflow-auto max-h-40">
                      {error.stack}
                    </pre>
                    {error.digest && (
                      <div className="flex items-center gap-2 text-xs text-border-light">
                        <span className="font-medium">ID d&apos;erreur:</span>
                        <code className="bg-background/50 px-2 py-1 rounded border border-border/30">
                          {error.digest}
                        </code>
                      </div>
                    )}
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button
              onClick={handleReset}
              className="bg-accent hover:bg-accent/90 text-black font-semibold px-8 py-3 rounded-xl shadow-lg hover:shadow-accent/25 transition-all duration-base hover:scale-105 flex items-center gap-2"
            >

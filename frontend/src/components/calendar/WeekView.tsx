...
export const WeekView: React.FC<WeekViewProps> = ({
  ...
  const timeSlots = useMemo(() => {
    const slots = [];
    for (let hour = 6; hour <= 20; hour++) { // 6 AM to 8 PM (extended hours)
  ...
  return (
    <div className={`bg-white overflow-x-auto ${className}`}>
      <div className="min-w-[800px]">
        {/* Header with days */}
        <div
          className="grid grid-cols-8 border-b sticky top-0 bg-white z-10 shadow-sm"
        ...
        >
          ...
          {weekData.map((day, index) => (
            ...
            >
              ...
              <div
                className={`
                  text-base font-bold rounded-full w-9 h-9 flex items-center justify-center mx-auto
                `}
                style={{
        ...
        <div className="relative">
          {timeSlots.map((timeSlot, _timeIndex) => (
            <div
              key={timeSlot}
                ...
                >
                  {(provided, snapshot) => (
                    <div
                      ...
                      className={`
                        min-h-[40px] p-1 border-r last:border-r-0
          ...
          {/* Tasks without specific time slots (at bottom) */}
          <div
              ...
              >
                {(provided, snapshot) => (
                  <div
                    ...
                    {...provided.droppableProps}
                    className={`
                      min-h-[60px] p-2 border-r last:border-r-0
                    `}
                    style={{
                      borderColor: designTokens.colors.border,
                      backgroundColor: snapshot.isDraggingOver ? designTokens.colors.surface : ''

    ...
    {showSkeleton ? (
      ...
      <>
        <div className="relative">
          <div className="w-16 h-16 border-4 border-[hsl(var(--rpma-teal))]/20 border-t-[hsl(var(--rpma-teal))] rounded-full animate-spin"></div>
          <div className="absolute inset-0 w-16 h-16 border-4 border-transparent border-t-[hsl(var(--rpma-teal))]/40 rounded-full animate-spin animation-delay-150"></div>
          <div className="absolute inset-0 w-16 h-16 border-4 border-transparent border-t-[hsl(var(--rpma-teal))]/60 rounded-full animate-spin animation-delay-300"></div>
        </div>
...
export const InlineLoading: React.FC<{ message?: string; size?: 'sm' | 'md' | 'lg' }> = ({
  ...
  return (
    <div className="flex items-center space-x-3 animate-in fade-in-0 duration-200">
      <div className="relative">
        <Loader2 className={`${sizeClasses[size]} animate-spin text-[hsl(var(--rpma-teal))]`} />
        <div className="absolute inset-0 rounded-full border-2 border-[hsl(var(--rpma-teal))]/30 animate-ping"></div>
      </div>
      {message && <span className="text-muted-foreground animate-in slide-in-from-right-2 duration-200 delay-100">{message}</span>}
    </div>
  );
};
...
export const ProgressIndicator: React.FC<{
  ...
  return (
    <div className="flex flex-col items-center space-y-3 animate-in fade-in-0 duration-300">
      ...
      <div className={`bg-[hsl(var(--rpma-surface))] rounded-full overflow-hidden ${sizeClasses[size].container}`}>
        <div
          className="h-full bg-gradient-to-r from-[hsl(var(--rpma-teal))] to-[hsl(var(--rpma-teal))] rounded-full transition-all duration-300 ease-out"

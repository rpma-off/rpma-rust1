...
export function TaskHistory({ taskId }: TaskHistoryProps) {
  ...
  const { data: changeLogs, isLoading, error } = useQuery<ChangeLog[]>({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/v1/tasks/${taskId}/history`);
      if (!response.ok) {
  ...
  if (!changeLogs || changeLogs.length === 0) {
    return (
      <div className="text-center py-8 text-muted-foreground">
        <Icons.History className="h-12 w-12 mx-auto mb-4 opacity-30" />
...
function getActionIcon(action: string) {
  ...
  switch (action) {
    ...
    case 'update':
      return <Icons.Edit3 {...iconProps} />;
    case 'delete':
      return <Icons.Trash {...iconProps} />;
    case 'assign':
      return <Icons.UserPlus {...iconProps} />;
    case 'complete':
      return <Icons.Check {...iconProps} />;
    default:
      return <Icons.History {...iconProps} />;
  }
}

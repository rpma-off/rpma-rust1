...
import { Client, ClientListResponse } from "@/lib/backend";
...
export const CustomerStep: React.FC<FormStepProps> = ({
  ...
  useEffect(() => {
    const fetchClients = async () => {
      if (!sessionToken) {
        console.error("No session token available for fetching clients");
        setClients([]);
        setFilteredClients([]);
        return;
      }
      ...
      try {
        // Import the IPC client dynamically to avoid SSR issues
        const { ipcClient } = await import('@/lib/ipc');
        const response = await ipcClient.clients.list({}, sessionToken);
        if (response && 'type' in response && response.type === 'List') {
          const typedResponse = response as ClientListResponse;
          const clientsList = Array.isArray(typedResponse.data) ? typedResponse.data : [];

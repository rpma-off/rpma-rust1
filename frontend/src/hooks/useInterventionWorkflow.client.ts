...
export function useInterventionWorkflow({
  ...
  const scheduleAutoSave = useCallback(() => {
    ...
    autoSaveTimeoutRef.current = setTimeout(() => {
      ...
      // Note: Actual auto-save logic should be implemented here
  ...
  const createIntervention = useCallback(async (data: StartInterventionDTO): Promise<InterventionCreationResponse> => {
    try {
      ...
      if (result.steps && result.steps.length > 0) {
        // First active step
        const firstActive = result.steps.find((s: { status?: string }) => s.status === 'in_progress') || result.steps[0];
  ...
  const validateCurrentStep = useCallback(async () => {
    ...
    if (currentStep.requires_photos && currentStep.photos && currentStep.min_photos_required && currentStep.photos.length < currentStep.min_photos_required) {
  ...
  const loadIntervention = useCallback(async (interventionId: string) => {
    try {
      ...
      const currentStep = steps.find((s: { status?: string }) => s.status === 'in_progress') ||
                          steps.find((s: { step_number?: number }) => s.step_number === intervention.completed_steps_count) ||
                          null;
  ...
  }, [handleError, loadIntervention, reset]);

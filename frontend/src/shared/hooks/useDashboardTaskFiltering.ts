...
interface DashboardTaskFilteringOptions {
  tasks: DashboardTask[];
  searchTerm: string;
  activeFilter: string;
...
export function useDashboardTaskFiltering({
  tasks,
  searchTerm,
  activeFilter,
  ...
  sortBy,
  sortOrder,
}: DashboardTaskFilteringOptions): DashboardTask[] {
  return useMemo(() => {
    ...
    if (selectedTechnicianFilter !== 'Tous les techniciens') {
      filtered = filtered.filter(task => task.technician?.name === selectedTechnicianFilter);
    }

...
test.describe('Intervention Management (PPF Workflow)', () => {
  ...
  test('should display PPF workflow page with start button', async ({ page }) => {
    await page.goto(WORKFLOW_URL, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await page.waitForLoadState('networkidle', { timeout: 30000 });

    await expect(page.getByText(/Workflow PPF|PPF/i).first()).toBeVisible({ timeout: 20000 });
    await expect(page.getByRole('button', { name: /Commencer/i }).first()).toBeVisible({ timeout: 10000 });
  });

  test('should start intervention and enter inspection step', async ({ page }) => {
    await page.goto(WORKFLOW_URL, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await page.waitForLoadState('networkidle', { timeout: 30000 });
    ...
    await expect(page.getByText(/Inspection/i).first()).toBeVisible({ timeout: 15000 });
  });
  ...
  test('should display inspection step details', async ({ page }) => {
    ...
    await expect(page.getByText(/Inspection initiale/i)).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should allow checking inspection checklist items', async ({ page }) => {
    ...
    await page.getByText('Véhicule propre et sec').click();
    await page.getByText('Température confirmée 18-25°C').click();

    // Verify items are visually checked
    const checkedItems = page.locator('[data-checked="true"], input[type="checkbox"]:checked, [aria-checked="true"]');
    await expect(checkedItems.first()).toBeVisible({ timeout: 5000 }).catch(() => {
  ...
  test('should track photo count after upload', async ({ page }) => {
    ...
    await expect(page.getByText(/2.*photos|photos.*2/i)).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should save inspection step as draft', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/steps\/inspection/, { timeout: 10000 });
  });

  test('should enable validate button after all requirements are met', async ({ page }) => {
    await page.goto(WORKFLOW_URL, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await page.waitForLoadState('networkidle', { timeout: 30000 });

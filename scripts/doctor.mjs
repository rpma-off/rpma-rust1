...
function getRustVersion() {
  try {
    const out = execSync('rustc --version', { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'], timeout: 5000 });
...
async function runCheck(name, category, cmd, argsArr, opts = {}) {
  ...
  const statusIcon = checkResult.status === 'pass' ? `${C.green}✓${C.reset}` : `${C.red}✗${C.reset}`;
...
async function checkCargoCheck() {
  if (FLAGS.quick) {
    return { name: 'cargo_check', category: 'backend', status: 'skipped', duration_ms: 0, message: 'Skipped (use without --quick)', fix_available: false };
  }
...
async function checkCargoTest() {
  if (!FLAGS.full) {
    return { name: 'cargo_test', category: 'backend', status: 'skipped', duration_ms: 0, message: 'Skipped (use --full)', fix_available: false };
  }
...
async function checkFrontendTypeCheck() {
  if (FLAGS.quick) {
    return { name: 'frontend_type_check', category: 'frontend', status: 'skipped', duration_ms: 0, message: 'Skipped (use without --quick)', fix_available: false };
  }
  return runCheck('frontend_type_check', 'frontend', 'npx', ['tsc', '--noEmit'], {
...
async function checkFrontendLint() {
  if (FLAGS.quick) {
    return { name: 'frontend_lint', category: 'frontend', status: 'skipped', duration_ms: 0, message: 'Skipped (use without --quick)', fix_available: false };
  }
  return runCheck('frontend_lint', 'frontend', 'npm', ['run', 'frontend:lint'], {
    fixAvailable: false,
    timeoutMs: 60000,
  });
}
...
async function checkFrontendTest() {
  if (!FLAGS.full) {
    return { name: 'frontend_test', category: 'frontend', status: 'skipped', duration_ms: 0, message: 'Skipped (use --full)', fix_available: false };
  }
...
async function checkSchemaDrift() {
  return runCheck('schema_drift', 'database', 'node', [join(ROOT, 'scripts', 'detect-schema-drift.js')], {
    fixAvailable: false,
    capture: true,
  });
}
...
async function checkArchitecture() {
  return runCheck('architecture', 'architecture', 'node', [join(ROOT, 'scripts', 'backend-architecture-check.js')], {
    fixAvailable: false,
    capture: true,
  });
}
...
async function checkMigrationSystem() {
  return runCheck('migration_system', 'database', 'node', [join(ROOT, 'scripts', 'validate-migration-system.js')], {
    fixAvailable: false,
    capture: true,
  });
}
...
results.summary.total = results.checks.length;
  ...
  results.summary.skipped = results.checks.filter(c => c.status === 'skipped').length;
  results.summary.warnings = results.checks.filter(c => c.status === 'warn').length;
...
if (FLAGS.fix && failed.length > 0) {
  ...
  if (outputJson) {
    results.status = results.summary.failed > 0 ? 'fail' : (results.summary.warnings > 0 ? 'warn' : 'pass');
    results.exit_code = results.summary.failed > 0 ? 1 : (results.summary.warnings > 0 ? 2 : 0);
    logJson(results);
  ...
  if (warnChecks.length > 0) {
    ...
    for (const c of warnChecks) {
      log(`  ${C.yellow}⚠${C.reset} ${c.name}: ${c.message || 'check details'}`);
    }
  }

...
impl PerformanceTestFixture {
    ...
    pub fn bulk_create_clients(&self, count: i32) -> AppResult<(Vec<Client>, Duration)> {
        ...
        for i in 0..count {
            ...
            let client = self.client_service.create_client_async(client_request, "perf_test_user").await?;
            clients.push(client);
        ...
        println!("Bulk created {} clients in {:?} (avg: {:?}/client)", 
    ...
    pub fn bulk_create_materials(&self, count: i32, initial_stock: f64) -> AppResult<(Vec<Material>, Duration)> {
        let start_time = Instant::now();
        ...
        for i in 0..count {
            let material_request = CreateMaterialRequest {
                ...
                material_type: if i % 3 == 0 { MaterialType::PpfFilm } else { MaterialType::Adhesive },
                ...
                specifications: None,
                unit_of_measure: if i % 2 == 0 { UnitOfMeasure::Meter } else { UnitOfMeasure::Liter },
        ...
        let duration = start_time.elapsed();
        println!("Bulk created {} materials in {:?} (avg: {:?}/material)", 
                count, duration, duration / count as u32);
    ...
    pub fn bulk_create_tasks(&self, clients: &[Client], tasks_per_client: i32) -> AppResult<(Vec<Task>, Duration)> {
        let start_time = Instant::now();
        ...
        let duration = start_time.elapsed();
        println!("Bulk created {} tasks in {:?} (avg: {:?}/task)", 
                tasks.len(), duration, duration / tasks.len() as u32);
        
        Ok((tasks, duration))
    }

    /// Bulk create interventions with timing
    pub async fn bulk_create_interventions(&self, tasks: &[Task]) -> AppResult<(Vec<Intervention>, Duration)> {
        let start_time = Instant::now();
        ...
        for (i, task) in tasks.iter().enumerate() {
            ...
            let intervention = self.intervention_service.get_intervention_by_id(&response.intervention_id)?;
        ...
        let duration = start_time.elapsed();
        println!("Bulk created {} interventions in {:?} (avg: {:?}/intervention)", 
                successful_count, duration, duration / successful_count as u32);
        
        Ok((interventions, duration))
    }

    /// Bulk consume materials with timing
    pub fn bulk_consume_materials(&self, interventions: &[Intervention], materials: &[Material]) -> AppResult<(Duration, f64)> {
        let start_time = Instant::now();
        ...
        for (i, intervention) in interventions.iter().enumerate() {
            ...
            for (step_idx, step) in intervention.steps.iter().enumerate().take(2) { // Limit to first 2 steps for performance
                ...
                match self.material_service.record_consumption(consumption_request) {
                    Ok(_) => {
                        total_consumed += 2.0;
        ...
        let duration = start_time.elapsed();
        println!("Bulk consumed materials in {:?} ({} consumptions, {:.1} total units)", 
    ...
    pub async fn test_concurrent_task_creation(&self, num_tasks: i32, concurrent_workers: i32) -> AppResult<(Duration, i32)> {
        let start_time = Instant::now();
        ...
        let duration = start_time.elapsed();
        println!("Concurrent task creation: {}/{} tasks in {:?} (avg: {:?}/task)", 
                successful_tasks, num_tasks, duration, duration / num_tasks as u32);
        
        Ok((duration, successful_tasks))
    }

    /// Test database query performance under load
    pub fn test_query_performance_under_load(&self, data_size: i32) -> AppResult<Vec<(String, Duration)>> {
        ...
        let query_scenarios = vec![
            ("Simple client query", Box::new(move || {
                // Simulate simple query
                Ok(vec![])
            }) as Box<dyn Fn() -> AppResult<Vec<Client>>>),
            ("Complex task query", Box::new(move || {
    ...
    pub fn measure_memory_usage(&self, operation_size: i32) -> AppResult<(Duration, u64)> {
        ...
        let (tasks, _) = self.bulk_create_tasks(&self.bulk_create_clients(operation_size / 10)?.0, 5)?;
        
        let final_memory = self.get_memory_usage()?;
        let duration = start_time.elapsed();
        
        println!("Memory usage: {} -> {} bytes (delta: {}) in {:?}", 
                initial_memory, final_memory, final_memory - initial_memory, duration);
    ...
    pub async fn test_transaction_performance(&self, batch_size: i32, num_batches: i32) -> AppResult<Duration> {
        let start_time = Instant::now();
        ...
        for batch_num in 0..num_batches {
            ...
            for i in 0..batch_size {
                ...
                let _client = self.client_service.create_client_async(client_request, "tx_test_user").await?;
            }
            
            let batch_duration = batch_start.elapsed();
            println!("Batch {} of {}: {:?} (avg: {:?}/record)", 
                    batch_num + 1, num_batches, batch_duration, batch_duration / batch_size as u32);
        }
        
        let total_duration = start_time.elapsed();
        let total_records = batch_size * num_batches;
        
        println!("Transaction performance: {} records in {:?} (avg: {:?}/record)", 
                total_records, total_duration, total_duration / total_records as u32);
        
        Ok(total_duration)
    }

    /// Test connection pooling performance
    pub async fn test_connection_pooling_performance(&self, num_concurrent_ops: i32) -> AppResult<(Duration, f64)> {
        let start_time = Instant::now();
        ...
        println!("Connection pooling: {}/{} operations in {:?} (success rate: {:.2}%)", 
                successful_ops, num_concurrent_ops, duration, success_rate * 100.0);
...
mod tests {
    ...
    async fn test_bulk_operations_performance() -> AppResult<()> {
        ...
        assert!(client_duration < Duration::from_secs(10), "Client creation too slow: {:?}", client_duration);
        assert!(material_duration < Duration::from_secs(15), "Material creation too slow: {:?}", material_duration);
        assert!(task_duration < Duration::from_secs(20), "Task creation too slow: {:?}", task_duration);
        
        // Test bulk interventions
        let (interventions, intervention_duration) = fixture.bulk_create_interventions(&tasks).await?;
        assert!(intervention_duration < Duration::from_secs(30), "Intervention creation too slow: {:?}", intervention_duration);
        
        // Test bulk material consumption
        let (consumption_duration, total_consumed) = fixture.bulk_consume_materials(&interventions, &materials)?;
        assert!(consumption_duration < Duration::from_secs(25), "Material consumption too slow: {:?}", consumption_duration);
    ...
    async fn test_concurrent_user_operations() -> AppResult<()> {
        ...
        // Test concurrent task creation
        let (concurrent_duration, successful_tasks) = fixture.test_concurrent_task_creation(200, 20).await?;
        
        assert!(concurrent_duration < Duration::from_secs(30), "Concurrent operations too slow: {:?}", concurrent_duration);
        assert!(successful_tasks > 180, "Too many failed concurrent operations: {}/200", successful_tasks);
        
        // Test connection pooling
        let (pool_duration, success_rate) = fixture.test_connection_pooling_performance(500).await?;
        
        assert!(pool_duration < Duration::from_secs(15), "Connection pooling too slow: {:?}", pool_duration);
        assert!(success_rate > 0.95, "Connection pool success rate too low: {:.2}%", success_rate * 100.0);
    ...
    async fn test_database_performance_under_load() -> AppResult<()> {
        ...
        for (scenario, duration) in query_results {
            assert!(duration < Duration::from_millis(500), 
                   "Query performance regression for {}: {:?}", scenario, duration);
        }
    ...
    async fn test_memory_usage_under_load() -> AppResult<()> {
        ...
        assert!(memory_duration < Duration::from_secs(60), "Memory test operations too slow: {:?}", memory_duration);
    ...
    async fn test_transaction_performance() -> AppResult<()> {
        ...
        assert!(tx_duration < Duration::from_secs(45), "Transaction operations too slow: {:?}", tx_duration);
    ...
    async fn test_performance_regression_detection() -> AppResult<()> {
        ...
        let (materials, material_baseline) = fixture.bulk_create_materials(baseline_data_size / 2, 50.0)?;
        let (tasks, task_baseline) = fixture.bulk_create_tasks(&clients, 2)?;
        let (interventions, intervention_baseline) = fixture.bulk_create_interventions(&tasks).await?;
        let (consumption_baseline, _) = fixture.bulk_consume_materials(&interventions, &materials)?;
        ...
        assert!(scaling_factor < 3.0, "Performance scaling regression: {:.2}x for 2x data", scaling_factor);
    ...
    async fn test_cross_domain_performance_integration() -> AppResult<()> {
        ...
        for (i, intervention) in interventions.iter().take(20).enumerate() {
            ...
            if let Some(step) = intervention.steps.first() {
                ...
                if fixture.material_service.record_consumption(consumption_request).is_ok() {
                    ...
                    let finalize_request = FinalizeInterventionRequest {
                        ...
                        final_observations: Some(vec!["Cross-domain performance completed".to_string()]),
                        customer_signature: None,
                        customer_comments: None,
                    };
                    
                    if fixture.intervention_service.finalize_intervention(
                        finalize_request, 
                        "xd_performance_test", 
        ...
        println!("Cross-domain performance: {} complete workflows in {:?} (avg: {:?}/workflow)", 
                completed_workflows, total_duration, total_duration / completed_workflows as u32);
        
        // Performance should be reasonable for complex cross-domain workflows
        assert!(total_duration < Duration::from_secs(60), 
               "Cross-domain workflow performance regression: {:?}", total_duration);
        assert!(completed_workflows > 10, "Too few completed workflows: {}/20", completed_workflows);

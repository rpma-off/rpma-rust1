...
impl ClientTaskInterventionMaterialFlowTestFixture {
    ...
    pub async fn convert_task_to_intervention(
        ...
        let intervention_request = StartInterventionRequest {
            ...
            notes: Some(format!("Intervention for task: {}", task.title.as_ref().unwrap_or(&String::new()))),
    ...
    pub async fn consume_materials_during_intervention(
        ...
        for (i, step) in intervention.steps.iter().enumerate() {
            ...
            self.intervention_service
                .advance_step(complete_request, "workflow-test", Some("workflow_technician"))
                .await?;
        }
    ...
    pub async fn finalize_intervention(
        ...
        self.intervention_service
            .finalize_intervention(finalize_request, "workflow-test", Some("workflow_technician"))
            .await?;
    ...
    pub fn get_client_statistics(&self, client_id: &str) -> AppResult<crate::services::client_statistics::ClientActivityMetrics> {
        self.client_stats_service.get_client_activity_metrics(client_id)
    }
    ...
    pub fn get_workflow_audit_trail(&self, client_id: &str, task_id: &str, intervention_id: &str) -> AppResult<Vec<crate::models::audit::AuditEvent>> {
        ...
        // Get client events
        let client_events = self.audit_service.get_resource_history("client", client_id, Some(20))?;
        all_events.extend(client_events);
        
        // Get task events
        let task_events = self.audit_service.get_resource_history("task", task_id, Some(20))?;
        all_events.extend(task_events);
        
        // Get intervention events
        let intervention_events = self.audit_service.get_resource_history("intervention", intervention_id, Some(20))?;
    ...
    pub fn verify_database_consistency(
        ...
        let client_exists: bool = conn
            .query_row("SELECT 1 FROM clients WHERE id = ?", [client_id], |_| Ok(true))
            .unwrap_or(false);
        
        // Verify task exists and has correct status
        let task_status: String = conn
            .query_row("SELECT status FROM tasks WHERE id = ?", [task_id], |row| row.get(0))
            .unwrap_or_default();
        
        // Verify intervention exists and has correct status
        let intervention_status: String = conn
            .query_row("SELECT status FROM interventions WHERE id = ?", [intervention_id], |row| row.get(0))
            .unwrap_or_default();
...
mod tests {
    ...
    async fn test_creating_a_task_for_a_client() -> AppResult<()> {
        ...
        assert_eq!(task.customer_address, Some(format!("{}, {}, {}, {}", 
            client.address_street.as_ref().unwrap_or(&String::new()),
            client.address_city.as_ref().unwrap_or(&String::new()),
            client.address_state.as_ref().unwrap_or(&String::new()),
            client.address_zip.as_ref().unwrap_or(&String::new())
        ...
        let task_events = fixture.audit_service.get_resource_history("task", &task.id, Some(10))?;
    ...
    async fn test_converting_task_to_intervention() -> AppResult<()> {
        ...
        let intervention_events = fixture.audit_service.get_resource_history("intervention", &intervention.id, Some(10))?;
        assert!(!intervention_events.is_empty());
        
        // Verify task audit trail was updated
        let task_events = fixture.audit_service.get_resource_history("task", &task.id, Some(10))?;
        assert!(task_events.iter().any(|e| e.action.contains("started") || e.action.contains("in_progress")));
    ...
    async fn test_consuming_materials_during_intervention() -> AppResult<()> {
        ...
        let updated_film = fixture.material_service.get_material_by_id(&materials[0].id.clone().unwrap())?;
        assert_eq!(updated_film.current_stock, initial_film_stock - 5.0 - 0.5);
        
        let updated_adhesive = fixture.material_service.get_material_by_id(&materials[1].id.clone().unwrap())?;
        ...
        for (i, consumption) in consumptions.iter().enumerate() {
            ...
            assert_eq!(consumption.waste_reason, Some("Workflow test waste".to_string()));
            assert_eq!(consumption.batch_used, Some(format!("BATCH-WF-{:03}", i + 1)));
            assert_eq!(consumption.quality_notes, Some("Good quality for workflow test".to_string()));
            assert_eq!(consumption.recorded_by, Some("workflow_technician".to_string()));
    ...
    async fn test_updating_client_statistics_based_on_completed_work() -> AppResult<()> {
        ...
        fixture.finalize_intervention(&intervention, Some(10), Some(95)).await?;
        ...
        fixture.consume_materials_during_intervention(&intervention2, &materials).await?;
        fixture.finalize_intervention(&intervention2, Some(8), Some(90)).await?;
    ...
    async fn test_audit_trail_creation_for_the_entire_flow() -> AppResult<()> {
        ...
        // 1. Client creation
        assert!(client_events.iter().any(|e| e.action.contains("create") || e.action.contains("created")));
        
        // 2. Task creation
        assert!(task_events.iter().any(|e| e.action.contains("create") || e.action.contains("created")));
        
        // 3. Task to intervention conversion
        assert!(task_events.iter().any(|e| e.action.contains("in_progress") || e.action.contains("started")));
        assert!(intervention_events.iter().any(|e| e.action.contains("create") || e.action.contains("started")));
        ...
        assert!(task_events.iter().any(|e| e.action.contains("complete") || e.action.contains("completed")));
        assert!(intervention_events.iter().any(|e| e.action.contains("complete") || e.action.contains("completed")));
    ...
    async fn test_complete_workflow_end_to_end() -> AppResult<()> {
        ...
        fixture.finalize_intervention(&intervention, Some(10), Some(98)).await?;
        ...
        let is_consistent = fixture.verify_database_consistency(&client.id, &task.id, &intervention.id)?;

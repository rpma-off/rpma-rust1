...
use crate::models::material::{Material, MaterialType, UnitOfMeasure, InventoryTransactionType};
...
use crate::services::material::{
    CreateMaterialRequest, CreateInventoryTransactionRequest, MaterialService,
...
impl InterventionMaterialTrackingTestFixture {
    ...
    pub fn create_materials_with_tracking(&self) -> AppResult<(Material, Material, Material)> {
        ...
        let film = self.material_service
        ...
        let adhesive = self.material_service
        ...
        let cleaner = self.material_service
    ...
    pub fn start_tracking_intervention(
        ...
        let intervention_request = StartInterventionRequest {
            ...
            ppf_zones: vec!["hood".to_string(), "fender".to_string(), "mirrors".to_string()],
            custom_zones: None,
    ...
    pub async fn record_material_usage_with_photos(
        ...
        self.material_service.record_consumption(cleaner_consumption)?;
        ...
        self.intervention_service
            .advance_step(complete_prep_request, "tracking-test", Some("test_technician"))
            .await?;
        ...
        self.intervention_service
            .advance_step(complete_film_request, "tracking-test", Some("test_technician"))
            .await?;
        ...
        self.material_service.record_consumption(adhesive_consumption)?;
        ...
        self.intervention_service
            .advance_step(complete_seal_request, "tracking-test", Some("test_technician"))
            .await?;
    ...
    pub fn get_consumption_history(&self, intervention_id: &str) -> AppResult<Vec<crate::models::material::MaterialConsumption>> {
    ...
    pub fn get_consumption_summary(&self, intervention_id: &str) -> AppResult<crate::models::material::InterventionMaterialSummary> {
        Ok(self.material_service.get_intervention_material_summary(intervention_id)?)
    }
}
...
mod tests {
    ...
    async fn test_materials_used_during_intervention_steps_are_tracked() -> AppResult<()> {
        ...
        fixture.record_material_usage_with_photos(&intervention, &film, &adhesive, &cleaner).await?;
        ...
        let cleaner_consumption = consumptions.iter().find(|c| c.material_id == cleaner.id.unwrap()).unwrap();
        assert_eq!(cleaner_consumption.quantity_used, 0.5);
        assert_eq!(cleaner_consumption.waste_quantity, 0.1);
        assert_eq!(cleaner_consumption.batch_used, Some("BATCH-CLN-2024-001".to_string()));
        assert_eq!(cleaner_consumption.step_number, Some(1));
        
        // Verify film usage
        let film_consumption = consumptions.iter().find(|c| c.material_id == film.id.unwrap()).unwrap();
        assert_eq!(film_consumption.quantity_used, 3.5);
        assert_eq!(film_consumption.waste_quantity, 0.7);
        assert_eq!(film_consumption.batch_used, Some("BATCH-FILM-2024-001".to_string()));
        assert_eq!(film_consumption.step_number, Some(2));
        assert_eq!(film_consumption.quality_notes, Some("Premium batch used for hood as requested".to_string()));
        
        // Verify adhesive usage
        let adhesive_consumption = consumptions.iter().find(|c| c.material_id == adhesive.id.unwrap()).unwrap();
        assert_eq!(adhesive_consumption.quantity_used, 0.2);
        assert_eq!(adhesive_consumption.waste_quantity, 0.02);
        assert_eq!(adhesive_consumption.batch_used, Some("BATCH-ADH-2024-001".to_string()));
    ...
    async fn test_material_waste_is_properly_recorded() -> AppResult<()> {
        ...
        assert_eq!(consumption.waste_reason, Some("Complex pattern trimming and adjustments".to_string()));
        
        // Verify stock was updated including waste
        let updated_film = fixture.material_service.get_material_by_id(&film.id.clone().unwrap())?;
        ...
        assert_eq!(film_consumption.waste_reason, Some("Complex pattern trimming and adjustments".to_string()));
        
        // Verify waste percentage calculation
        let waste_percentage = (film_consumption.waste_quantity / (film_consumption.quantity_used + film_consumption.waste_quantity)) * 100.0;
    ...
    async fn test_intervention_completion_calculates_total_material_costs() -> AppResult<()> {
        ...
        let finalize_request = FinalizeInterventionRequest {
            ...
            final_observations: Some(vec!["High quality installation with premium materials".to_string()]),
        ...
        let expected_total_cost = expected_film_cost + expected_adhesive_cost + expected_cleaner_cost;
        
        assert!((summary.total_cost - expected_total_cost).abs() < 0.01);
        
        // Verify individual material costs
        let film_summary = summary.materials.iter().find(|m| m.material_id == film.id.unwrap()).unwrap();
        assert!((film_summary.total_cost.unwrap() - expected_film_cost).abs() < 0.01);
        
        let adhesive_summary = summary.materials.iter().find(|m| m.material_id == adhesive.id.unwrap()).unwrap();
        assert!((adhesive_summary.total_cost.unwrap() - expected_adhesive_cost).abs() < 0.01);
        
        let cleaner_summary = summary.materials.iter().find(|m| m.material_id == cleaner.id.unwrap()).unwrap();
    ...
    async fn test_photos_attached_to_intervention_steps_with_material_usage() -> AppResult<()> {
        ...
        fixture.material_service.record_consumption(consumption_request)?;
        ...
        let updated_step = fixture.intervention_service
        ...
        let updated_intervention = fixture.intervention_service.get_intervention_by_id(&intervention.id)?;
        let updated_step_in_intervention = updated_intervention.steps.iter().find(|s| s.id == step.id).unwrap();
        assert!(updated_step_in_intervention.photos.is_some());
        assert!(!updated_step_in_intervention.photos.as_ref().unwrap().is_empty());
    ...
    async fn test_batch_and_expiry_tracking_for_materials_used() -> AppResult<()> {
        ...
        let film_consumption = fixture.material_service.record_consumption(film_consumption_request)?;
        ...
        let adhesive_consumption = fixture.material_service.record_consumption(adhesive_consumption_request)?;
        ...
        let film_history = consumptions.iter().find(|c| c.material_id == film.id.unwrap()).unwrap();
        assert_eq!(film_history.batch_used, Some("BATCH-FILM-2024-001".to_string()));
        
        // Verify adhesive consumption details
        let adhesive_history = consumptions.iter().find(|c| c.material_id == adhesive.id.unwrap()).unwrap();
        assert_eq!(adhesive_history.batch_used, Some("BATCH-ADH-2024-001".to_string()));
        
        // Verify that we can trace back to the material's batch and expiry information
        let film_material = fixture.material_service.get_material_by_id(&film.id.unwrap())?;
        assert_eq!(film_material.batch_number, Some("BATCH-FILM-2024-001".to_string()));
        assert!(film_material.expiry_date.is_some());
        
        let adhesive_material = fixture.material_service.get_material_by_id(&adhesive.id.unwrap())?;
        assert_eq!(adhesive_material.batch_number, Some("BATCH-ADH-2024-001".to_string()));
        ...
        fixture.intervention_service
            .advance_step(complete_request, "batch-expiry-test", Some("test_technician"))
            .await?;
        
        // Verify the collected data includes batch/expiry verification
        let updated_intervention = fixture.intervention_service.get_intervention_by_id(&intervention.id)?;
        let updated_step = updated_intervention.steps.iter().find(|s| s.id == step.id).unwrap();

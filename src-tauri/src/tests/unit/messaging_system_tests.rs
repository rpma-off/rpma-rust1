...
use crate::models::message::{
    ...
    MessageQuery, MessageListResponse
};
...
async fn test_message_repository_crud() {
    ...
    // Create test users
    create_test_user(&repo.db.get_connection().unwrap(), "sender_1", "sender1@example.com");
    create_test_user(&repo.db.get_connection().unwrap(), "recipient_1", "recipient1@example.com");
    ...
    // Update status
    repo.update_status("msg_test_1", MessageStatus::Sent).await.unwrap();
    
    // Verify status update
    let updated = repo.find_by_id("msg_test_1".to_string()).await.unwrap().unwrap();
...
async fn test_message_type_filtering() {
    ...
    // Create test users
    create_test_user(&repo.db.get_connection().unwrap(), "sender_types", "sender@example.com");
    create_test_user(&repo.db.get_connection().unwrap(), "recipient_types", "recipient@example.com");
...
async fn test_message_status_transitions() {
    ...
    // Create test users
    create_test_user(&repo.db.get_connection().unwrap(), "sender_status", "sender@example.com");
    create_test_user(&repo.db.get_connection().unwrap(), "recipient_status", "recipient@example.com");
    ...
    // Pending -> Sent
    repo.update_status("msg_status", MessageStatus::Sent).await.unwrap();
    let message = repo.find_by_id("msg_status".to_string()).await.unwrap().unwrap();
    assert_eq!(message.status, "sent");
    assert!(message.sent_at.is_some());
    assert!(message.sent_at.unwrap() >= now);
    
    // Sent -> Delivered
    repo.update_status("msg_status", MessageStatus::Delivered).await.unwrap();
    let message = repo.find_by_id("msg_status".to_string()).await.unwrap().unwrap();
    assert_eq!(message.status, "delivered");
    assert!(message.sent_at.is_some()); // Should retain sent_at
    
    // Delivered -> Read
    repo.update_status("msg_status", MessageStatus::Read).await.unwrap();
    let message = repo.find_by_id("msg_status".to_string()).await.unwrap().unwrap();
    ...
    repo.update_status("msg_status", MessageStatus::Failed).await.unwrap();
    let message = repo.find_by_id("msg_status".to_string()).await.unwrap().unwrap();
...
async fn test_message_recipient_filtering() {
    ...
    create_test_user(&repo.db.get_connection().unwrap(), "sender_recipient", "sender@example.com");
    create_test_user(&repo.db.get_connection().unwrap(), "recipient_a", "recipient_a@example.com");
    create_test_user(&repo.db.get_connection().unwrap(), "recipient_b", "recipient_b@example.com");
    
    // Create messages for different recipients
    for i in 0..5 {
        let recipient_id = if i % 2 == 0 { "recipient_a" } else { "recipient_b" };
...
async fn test_unsent_messages_filter() {
    ...
    // Create test users
    create_test_user(&repo.db.get_connection().unwrap(), "sender_unsent", "sender@example.com");
    create_test_user(&repo.db.get_connection().unwrap(), "recipient_unsent", "recipient@example.com");
    
    // Create messages with different statuses
    for (i, status) in ["pending", "sent", "failed", "delivered"].iter().enumerate() {
        let message = Message {
            ...
            priority: "normal".to_string(),
            scheduled_at: None,
            sent_at: if *status == "sent" { Some(chrono::Utc::now().timestamp()) } else { None },
            read_at: None,
            error_message: None,
            metadata: None,
            created_at: chrono::Utc::now().timestamp(),
...
async fn test_message_date_filtering() {
    ...
    // Create test users
    create_test_user(&repo.db.get_connection().unwrap(), "sender_date", "sender@example.com");
    create_test_user(&repo.db.get_connection().unwrap(), "recipient_date", "recipient@example.com");
    ...
    let recent_messages = repo.search(MessageQuery {
    ...
    let old_messages = repo.search(MessageQuery {
...
async fn test_message_template_crud() {
    ...
    assert_eq!(template.subject, Some("New Task Assigned: {{task_number}}".to_string()));
    ...
    assert_eq!(updated_template.subject, Some("Updated: {{task_number}}".to_string()));
    
    // Delete template
    let rows_affected = db
        .execute("DELETE FROM message_templates WHERE id = ?", params![template_id])
        .unwrap();
...
async fn test_message_scheduling() {
    ...
    let saved_message = repo.find_by_id("msg_scheduled".to_string()).await.unwrap().unwrap();
    ...
    let unsent_now = repo.search(MessageQuery {
        ..Default::default()
    }).await.unwrap();
...
async fn test_message_error_handling() {
    ...
    // Create test users
    create_test_user(&repo.db.get_connection().unwrap(), "sender_error", "sender@example.com");
    create_test_user(&repo.db.get_connection().unwrap(), "recipient_error", "recipient@example.com");
    ...
    repo.update_status("msg_error", MessageStatus::Failed).await.unwrap();
    
    // In a real implementation, we would also update the error_message
    // For now, we verify the status transition
    let failed_message = repo.find_by_id("msg_error".to_string()).await.unwrap().unwrap();

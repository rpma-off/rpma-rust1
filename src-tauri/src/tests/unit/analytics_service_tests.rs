...
async fn test_get_active_kpis() {
    ...
    for kpi in &kpis {
        ...
        assert!(!kpi.display_name.is_empty(), "KPI display name should not be empty");
        assert!(!kpi.calculation_method.is_empty(), "KPI calculation method should not be empty");
    }
}
...
async fn test_calculate_task_completion_rate() {
    ...
    assert!(updated_kpi.current_value.is_some(), "Current value should be set");
    assert_eq!(updated_kpi.current_value.unwrap(), 50.0, "Completion rate should be 50%");
}
...
async fn test_calculate_avg_completion_time() {
    ...
    db.execute_batch(r#"
        INSERT INTO interventions (id, status, started_at, completed_at) VALUES
            ...
            ('int_2', 'completed', ?, ?);
    "#, params![
        four_hours_ago, two_hours_ago,  // 2 hour duration
        four_hours_ago, now  // 4 hour duration
    ]).unwrap();
    ...
    assert!(updated_kpi.current_value.is_some(), "Current value should be set");
    assert_eq!(updated_kpi.current_value.unwrap(), 3.0, "Average completion time should be 3 hours");
}
...
async fn test_calculate_first_time_fix_rate() {
    ...
    assert_eq!(rate, 66.66666666666667, "First time fix rate should be 66.67% (2 out of 3)");
}
...
async fn test_calculate_inventory_turnover() {
    ...
    // 65 / 150 = 0.433333...
    assert!((turnover - 0.433333).abs() < 0.001, "Inventory turnover should be ~0.433");
}
...
async fn test_get_analytics_summary() {
    ...
    assert_eq!(summary.total_interventions, 3, "Should have 3 total interventions");
    ...
    assert!(summary.average_completion_time >= 0.0, "Average completion time should be non-negative");
    assert!(summary.client_satisfaction_score >= 0.0, "Client satisfaction should be non-negative");
    assert!(summary.quality_compliance_rate >= 0.0, "Quality compliance should be non-negative");
    assert!(summary.inventory_turnover >= 0.0, "Inventory turnover should be non-negative");
}
...
async fn test_get_metric_time_series() {
    ...
    let time_series = service.get_metric_time_series("task_completion_rate", 30).unwrap();
    ...
    assert_eq!(time_series.data_points.len(), 3, "Should have 3 data points");
...
async fn test_calculate_all_kpis() {
    ...
    for kpi in kpis {
        assert!(
            kpi.current_value.is_some() || kpi.kpi_name == "client_satisfaction_score" || kpi.kpi_name == "revenue_per_technician_hour",
...
async fn test_trend_direction_calculation() {
    ...
    assert_eq!(trend, Some(TrendDirection::Unknown), "Trend should be unknown with no previous value");
    
    // Test with significant increase
    let trend = service.calculate_trend_direction(60.0, Some(50.0));
    assert_eq!(trend, Some(TrendDirection::Up), "Trend should be up with significant increase");
    
    // Test with significant decrease
    let trend = service.calculate_trend_direction(40.0, Some(50.0));
    assert_eq!(trend, Some(TrendDirection::Down), "Trend should be down with significant decrease");
    
    // Test with small change (within 5% threshold)
    let trend = service.calculate_trend_direction(51.0, Some(50.0));
    assert_eq!(trend, Some(TrendDirection::Stable), "Trend should be stable with small change");
    
    // Test with exact same value
    let trend = service.calculate_trend_direction(50.0, Some(50.0));
    assert_eq!(trend, Some(TrendDirection::Stable), "Trend should be stable with same value");
}
...
async fn test_unknown_kpi_handling() {
    ...
    assert!(result.unwrap_err().contains("Unknown KPI"), "Error should mention unknown KPI");
}
...
use rpma_ppf_intervention::repositories::{TaskRepository, MaterialRepository, InterventionRepository};
use rpma_ppf_intervention::models::{Task, CreateTaskRequest, TaskStatus, TaskPriority};
...
async fn test_intervention_material_join_performance() {
    ...
    let start = Instant::now();
    let _interventions = intervention_repo
        .find_with_materials(0, 50)
        .await
        .unwrap();
    let duration = start.elapsed();
...
async fn create_test_task_with_status(task_repo: &TaskRepository, number: i32) -> Task {
    let request = CreateTaskRequest {
        ...
        description: Some("Test description".to_string()),
        priority: TaskPriority::Normal,
        status: if number % 2 == 0 { TaskStatus::Pending } else { TaskStatus::Completed },
        client_id: "test-client".to_string(),
        ppf_zone: "ZONE-001".to_string(),
        estimated_duration_hours: Some(2.0),
        scheduled_date: None,
...
async fn create_test_material(material_repo: &MaterialRepository) -> rpma_ppf_intervention::models::Material {

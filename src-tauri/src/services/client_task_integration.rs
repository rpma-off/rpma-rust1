...
impl ClientTaskIntegrationService {
    ...
    pub fn reassign_tasks(&self, from_client_id: &str, to_client_id: &str, user_id: &str) -> Result<i32, String> {
    ...
    pub fn handle_client_deletion(&self, client_id: &str, cascade: bool) -> Result<ClientDeletionResult, String> {
        // Check for existing tasks
        let task_count: i64 = self.db
            .as_ref()
            .query_single_value(
    ...
    pub fn get_client_workload_distribution(&self) -> Result<Vec<ClientWorkload>, String> {
        ...
        let mut results = Vec::new();
        for (client_id, client_name, total_tasks, pending_tasks, in_progress_tasks, completed_tasks, avg_estimated_duration) in rows {

...
use crate::models::client::{
    ClientListResponse, ClientQuery, Client,
};
use rusqlite::params;
use std::sync::Arc;
...
impl ClientQueriesService {
    ...
    pub fn get_clients(&self, query: ClientQuery) -> Result<ClientListResponse, String> {
        ...
        let valid_sort_columns = [
            "name", "email", "created_at", "updated_at", "total_tasks",
            "active_tasks", "completed_tasks", "customer_type"
        ];
    ...
    pub fn get_clients_by_type(&self, customer_type: crate::models::CustomerType) -> Result<Vec<Client>, String> {
        let sql = r#"
            SELECT
                c.id, c.name, c.email, c.phone, c.customer_type, c.address_street, c.address_city,
                c.address_state, c.address_zip, c.address_country, c.tax_id, c.company_name,
    ...
    pub fn get_recently_active_clients(&self, days: i32) -> Result<Vec<Client>, String> {
        let cutoff_timestamp = chrono::Utc::now().timestamp_millis() - (days as i64 * 24 * 60 * 60 * 1000);

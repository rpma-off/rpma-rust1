...
impl CalendarService {
    ...
    pub async fn get_tasks(
        ...
        let mut sql = String::from(
            "SELECT * FROM calendar_tasks WHERE scheduled_date BETWEEN ?1 AND ?2"
        );
        ...
        if let Some(tech_ids) = &technician_ids {
            if !tech_ids.is_empty() {
                let placeholders = tech_ids.iter()
                    .map(|_| "?")
                    .collect::<Vec<_>>()
                    .join(",");
        ...
        if let Some(statuses) = &statuses {
            if !statuses.is_empty() {
                let placeholders = statuses.iter()
                    .map(|_| "?")
                    .collect::<Vec<_>>()
                    .join(",");
        ...
        let tasks = stmt.query_map(rusqlite::params_from_iter(params), |row| {
            Ok(CalendarTask {
                id: row.get(0)?,
    ...
    pub async fn check_conflicts(
        ...
        let technician_id: Option<String> = conn.query_row(
        ...
        Ok(ConflictDetection {
            has_conflict,
            conflict_type: if has_conflict { Some("time_overlap".to_string()) } else { None },

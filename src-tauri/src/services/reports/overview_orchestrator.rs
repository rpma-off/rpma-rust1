...
use super::{
    ...
    client_report::generate_client_analytics_report,
    quality_report::generate_quality_compliance_report,
    material_report::generate_material_usage_report,
    geographic_report::generate_geographic_report,
...
pub async fn generate_overview_report(
    ...
    let technician_performance = generate_technician_performance_report(date_range, None, db).await?;
    ...
    let operational_intelligence = generate_operational_intelligence_report(date_range, filters, db).await?;
...
/// Get intervention with details (placeholder - needs migration from old code)
...
pub async fn get_entity_counts(db: &Database) -> AppResult<EntityCounts> {
    ...
    let task_count = db
        .count_rows("tasks")
        .map_err(|e| crate::commands::AppError::Database(format!("Failed to count tasks: {}", e)))?;

    // Get client count
    let client_count = db
        .count_rows("clients")
        .map_err(|e| crate::commands::AppError::Database(format!("Failed to count clients: {}", e)))?;

    // Get intervention count (steps table)
    let intervention_count = db
        .count_rows("steps")
        .map_err(|e| crate::commands::AppError::Database(format!("Failed to count interventions: {}", e)))?;

...
impl ExportReportService {
    ...
    pub async fn get_intervention_with_details(
        ...
        let intervention_svc = match intervention_service {
            ...
            None => {
                owned_intervention_service =
                    crate::services::intervention::InterventionService::new(
            ...
            None => {
                #[allow(deprecated)]
                {
                    owned_client_service =
                        crate::services::client::ClientService::new_with_db(
                            std::sync::Arc::new(db.clone()),
                        );
                }
                &owned_client_service
        ...
        let workflow_steps = intervention_svc
            ...
            .map_err(|e| {
                AppError::Database(format!("Failed to get workflow steps: {}", e))
            })?;
        info!(
        ...
        let photos = intervention_svc
            ...
            .map_err(|e| {
                AppError::Database(format!("Failed to get intervention photos: {}", e))
            })?;
        info!(
    ...
    pub async fn generate_task_completion_csv(
        ...
        let json_value = serde_json::to_value(&report.daily_breakdown).map_err(|e| {
            AppError::Internal(format!("Failed to serialize report data: {}", e))
        })?;

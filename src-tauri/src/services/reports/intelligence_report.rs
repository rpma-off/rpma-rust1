...
pub async fn generate_operational_intelligence_report(
    ...
    for (step_number, step_name, step_type, avg_duration, total_steps, failed_count, rework_count, paused_count) in step_bottlenecks_data {
        let total_steps_f64 = total_steps as f64;
        let average_duration_minutes = avg_duration.map(|d| d / 60.0).unwrap_or(0.0);
        let failure_rate = if total_steps > 0 { (failed_count as f64 / total_steps_f64) * 100.0 } else { 0.0 };
        let rework_rate = if total_steps > 0 { (rework_count as f64 / total_steps_f64) * 100.0 } else { 0.0 };
        let pause_rate = if total_steps > 0 { (paused_count as f64 / total_steps_f64) * 100.0 } else { 0.0 };
    ...
    let intervention_bottlenecks_data: Vec<(String, String, i32, Option<i32>, i32, f64, String)> = db
        .query_multiple(
    ...
    for (id, vehicle_plate, actual_duration, estimated_duration, current_step, _completion_percentage, _status) in intervention_bottlenecks_data {
    ...
    for (technician_id, technician_name, total_tasks, completed_tasks, avg_duration, _total_duration) in resource_data {
        let utilization_percentage = if total_tasks > 0 { (completed_tasks as f64 / total_tasks as f64) * 100.0 } else { 0.0 };
    ...
    let total_steps = step_bottlenecks.iter().map(|s| s.total_occurrences).sum::<u64>() as f64;
    let failed_steps = step_bottlenecks.iter().map(|s| s.failure_rate / 100.0 * s.total_occurrences as f64).sum::<f64>();
    let rework_steps = step_bottlenecks.iter().map(|s| s.rework_rate / 100.0 * s.total_occurrences as f64).sum::<f64>();

    let overall_efficiency_score = if total_steps > 0.0 { ((total_steps - failed_steps - rework_steps) / total_steps) * 100.0 } else { 0.0 };
    let average_step_completion_time = step_bottlenecks.iter().map(|s| s.average_duration_minutes).sum::<f64>() / step_bottlenecks.len() as f64;
    let step_success_rate = if total_steps > 0.0 { ((total_steps - failed_steps) / total_steps) * 100.0 } else { 0.0 };
    let rework_percentage = if total_steps > 0.0 { (rework_steps / total_steps) * 100.0 } else { 0.0 };
    let resource_utilization_rate = resource_utilization.iter().map(|r| r.utilization_percentage).sum::<f64>() / resource_utilization.len() as f64;
    let bottleneck_impact_score = step_bottlenecks.iter().filter(|s| s.bottleneck_severity == "high").count() as f64 * 10.0;
    ...
    if overall_efficiency_score < 80.0 {
        recommendations.push(WorkflowRecommendation {
            ...
            description: format!("Overall efficiency score is {:.1}%. Focus on reducing failures and rework.", overall_efficiency_score),
    ...
    if resource_utilization_rate < 70.0 {
        recommendations.push(WorkflowRecommendation {
            ...
            priority: "medium".to_string(),
            description: format!("Average resource utilization is {:.1}%. Consider workload balancing.", resource_utilization_rate),
            ...
            affected_technicians: resource_utilization.iter().map(|r| r.technician_id.clone()).collect(),
        });
    }
    ...
    let process_efficiency = ProcessEfficiencyMetrics {
        ...
        average_step_completion_time: if average_step_completion_time.is_finite() { average_step_completion_time } else { 0.0 },
        step_success_rate,
        rework_percentage,
        resource_utilization_rate: if resource_utilization_rate.is_finite() { resource_utilization_rate } else { 0.0 },

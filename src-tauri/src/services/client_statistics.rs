...
impl ClientStatisticsService {
    ...
    pub fn get_client_stats(&self) -> Result<ClientStats, String> {
        ...
        let active_clients: i32 = self.db
            .as_ref()
            .query_single_value("SELECT COUNT(DISTINCT c.id) FROM clients c
                 INNER JOIN tasks t ON c.id = t.client_id
                 WHERE c.deleted_at IS NULL AND t.created_at >= ? AND t.deleted_at IS NULL", rusqlite::params![ninety_days_ago])
        ...
        let new_clients_this_month: i32 = self.db
            .as_ref()
            .query_single_value("SELECT COUNT(*) FROM clients WHERE created_at >= ? AND deleted_at IS NULL", rusqlite::params![start_of_month])
            .unwrap_or(0);
    ...
    pub fn get_client_activity_metrics(&self, client_id: &str) -> Result<ClientActivityMetrics, String> {
        // Get task statistics
        let sql = r#"
            SELECT
    ...
    pub fn get_geographic_stats(&self) -> Result<ClientGeographicStats, String> {
        ...
        let conn = self.db.as_ref().get_connection()?;
        let mut stmt = conn.prepare(state_sql).map_err(|e| format!("Failed to prepare statement: {}", e))?;
        let state_rows = stmt
            .query_map([], |row| {
        ...
        let conn = self.db.as_ref().get_connection()?;
        let mut stmt = conn.prepare(city_sql).map_err(|e| format!("Failed to prepare statement: {}", e))?;
        let city_rows = stmt
            .query_map([], |row| {
                Ok((row.get::<_, String>(0)?, row.get::<_, i32>(1)?))
            })
        ...
        let mut stmt = conn.prepare(region_sql).map_err(|e| format!("Failed to prepare statement: {}", e))?;
    ...
    pub fn get_client_lifecycle_report(&self, client_id: &str) -> Result<ClientLifecycleReport, String> {
        // Get client creation date
        let created_at: i64 = self.db
            .as_ref()
            .query_single_value(
        ...
        let average_task_frequency = if let (Some(first), Some(last)) = (first_task_date, last_task_date) {
            if last > first {
    ...
    pub fn get_top_clients_by_activity(&self, limit: i32) -> Result<Vec<ClientActivityMetrics>, String> {
        let sql = r#"
            SELECT
                c.id,

...
pub fn validate_save_path(file_path: &str) -> AppResult<()> {
    ...
    if file_path.contains("\\\\Windows\\\\") || file_path.contains("\\\\System32\\\\") ||
        file_path.contains("\\\\Program Files\\\\") || file_path.starts_with("C:\\\\") && file_path.len() < 10 {
        return Err(crate::commands::errors::AppError::Validation(
    ...
    if path.exists() {
        match std::fs::OpenOptions::new().write(true).open(path) {
            ...
            Err(e) => {
                return Err(crate::commands::errors::AppError::Validation(
                    format!("Cannot overwrite existing file {}: {}", path.display(), e),
                ));
            }
        }
    }
...
pub async fn save_pdf_to_path(
    ...
    info!("Estimated PDF size: {} MB ({} bytes) for intervention with {} photos and {} steps",
          estimated_mb, estimated_size, intervention_data.photos.len(), intervention_data.workflow_steps.len());
    ...
    if let Ok(memory_stats) = memory_management_helpers::get_memory_stats() {
        ...
        let estimated_total_mb = (memory_stats.total_memory_used as f64 / memory_stats.memory_usage_percent.max(0.01) / 100.0) as u64;
        let available_mb = (estimated_total_mb.saturating_sub(memory_stats.total_memory_used)) / (1024 * 1024);
        ...
        if (available_mb as usize) < estimated_mb + safety_margin_mb {
            ...
            return Err(crate::commands::errors::AppError::Internal(
                format!("Insufficient memory for PDF generation ({} MB needed, {} MB available)",
                        estimated_mb + safety_margin_mb, available_mb)
            ));
        }

        // Warn for large interventions that might cause performance issues
        if estimated_mb > 50 {
            warn!("Large intervention detected ({} MB) - save operation may be slow", estimated_mb);
        }

        // Automatic cleanup if memory pressure detected
        if memory_stats.memory_usage_percent > 85.0 {
            warn!("Memory pressure detected ({}%) - triggering automatic cleanup", memory_stats.memory_usage_percent);
    ...
    if intervention_data.photos.len() > 100 {
        warn!("Intervention {} has {} photos - this may cause memory issues", intervention_data.intervention.id, intervention_data.photos.len());
        return Err(crate::commands::errors::AppError::Validation(
            format!("Intervention has too many photos ({}). Maximum supported: 100", intervention_data.photos.len())
        ));
    }

    if intervention_data.workflow_steps.len() > 200 {
        warn!("Intervention {} has {} workflow steps - this may cause memory issues", intervention_data.intervention.id, intervention_data.workflow_steps.len());
        return Err(crate::commands::errors::AppError::Validation(
            format!("Intervention has too many workflow steps ({}). Maximum supported: 200", intervention_data.workflow_steps.len())
        ));
    }

    // CRASH PROTECTION: Generate PDF report with timeout protection
    info!("Generating PDF report for save operation (estimated size: {} MB)", estimated_mb);
    ...
    let pdf_result = match tokio::time::timeout(
        ...
        Err(_) => {
            ...
            return Err(crate::commands::errors::AppError::Internal(
                format!("PDF generation timed out after {:?}", pdf_timeout_duration)
            ));
        }
    };
    ...
    if let Some(generated_path) = pdf_result.file_path {
        info!("Copying generated PDF from {} to {} ({} bytes)", generated_path, file_path, pdf_result.file_size.unwrap_or(0));
        ...
        let source_metadata = match source_path.metadata() {
            ...
            Err(e) => {
                ...
                return Err(crate::commands::errors::AppError::Internal(
                    format!("Cannot access generated PDF file: {}", e)
                ));
            }
        };
                ...
                Ok(available) => {
                    ...
                    if available < (expected_size + safety_margin) {
                        ...
                        return Err(crate::commands::errors::AppError::Io(
                            format!("Insufficient disk space ({} MB available, {} MB needed)",
                                    available / (1024 * 1024), (expected_size + safety_margin) / (1024 * 1024))
                        ));
                    }
                }
                Err(e) => {
        ...
        match std::fs::copy(&generated_path, file_path) {
            Ok(bytes_copied) => {
                info!("File copy operation succeeded: {} bytes copied", bytes_copied);
                if bytes_copied != expected_size {
                    error!("File copy size mismatch: expected {}, copied {}", expected_size, bytes_copied);
                    // Attempt to remove the incomplete destination file
                    let _ = std::fs::remove_file(file_path);
                    let _ = std::fs::remove_file(&generated_path);
                    return Err(crate::commands::errors::AppError::Io(
                        format!("File copy incomplete: expected {} bytes, copied {} bytes", expected_size, bytes_copied)
                    ));
                }

                info!("PDF report copied successfully to: {} ({} bytes)", file_path, bytes_copied);
                ...
                match dest_path.metadata() {
                    Ok(dest_metadata) => {
                        if dest_metadata.len() == expected_size {
                            ...
                            if let Err(cleanup_err) = std::fs::remove_file(&generated_path) {
                                error!("Failed to cleanup temporary file {}: {}", generated_path, cleanup_err);
                                // Don't fail the operation for cleanup errors
                            }
                        } else {
                            error!("Destination file size mismatch after copy: expected {}, got {}", expected_size, dest_metadata.len());
                            let _ = std::fs::remove_file(file_path);
                            let _ = std::fs::remove_file(&generated_path);
                            return Err(crate::commands::errors::AppError::Io(
                    ...
                    Err(e) => {
                        ...
                        let _ = std::fs::remove_file(&generated_path);
                        return Err(crate::commands::errors::AppError::Io(
                            format!("Destination file verification failed: {}", e)
                        ));
                    }
                }
            }
            Err(e) => {
                ...
                let error_msg = if e.kind() == std::io::ErrorKind::PermissionDenied {
                    ...
                    format!("Path not found: The directory for '{}' does not exist.", file_path)
...
async fn generate_pdf_with_timeout(
    ...
    let file_name = DocumentStorageService::generate_filename(
        &format!("intervention_report_{}_{}", intervention_data.intervention.id, chrono::Utc::now().timestamp()),
        "pdf",
    );

...
use crate::domains::quotes::ipc::quote::{QuoteCreateRequest, create_quote};
...
async fn test_create_quote_unauthenticated_fails_with_auth_error() -> AppResult<()> {
    ...
    assert!(err_msg.contains("Session not found") || err_msg.contains("Unauthorized") || err_msg.contains("auth"), "Expected auth error, got: {}", err_msg);

...
use rpma_ppf_intervention::models::{Task, CreateTaskRequest, UpdateTaskRequest, TaskQuery, TaskPriority, TaskStatus};
...
async fn test_task_create_valid() {
    ...
    let client_response = rpma_ppf_intervention::commands::client_crud(client_req).await.unwrap();
...
async fn test_task_create_invalid_status_transition() {
    ...
    assert!(response.error.unwrap().message.to_lowercase().contains("invalid") || 
            response.error.unwrap().message.to_lowercase().contains("transition"));
}
...
async fn test_task_assignment_technician_overload() {
    ...
    assert!(!response.success);
    assert!(response.error.is_some());
    assert!(response.error.unwrap().message.to_lowercase().contains("overload") || 
            response.error.unwrap().message.to_lowercase().contains("maximum"));
}
...
async fn test_task_update_valid() {
    ...
    let result = task_crud(rpma_ppf_intervention::commands::TaskAction::Update {
        id: task.id,
        data: update_req,
    }, session_token).await;
...
async fn test_task_statistics() {
    ...
    for (i, status) in [TaskStatus::Pending, TaskStatus::InProgress, TaskStatus::Completed].iter().enumerate() {
        let task_req = CreateTaskRequest {
            title: format!("Task {}", i),
            description: "Description".to_string(),
            client_id: client_id.to_string(),

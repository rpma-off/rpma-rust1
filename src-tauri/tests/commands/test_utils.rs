...
pub async fn create_test_db() -> TestContext {
    ...
    // Create repositories
    let repos = Arc::new(repositories::Repositories::new(&db).expect("Failed to create repositories"));
    
    // Create services
    let auth_service = Arc::new(rpma_ppf_intervention::services::AuthService::new(db.clone()));
    let session_service = Arc::new(rpma_ppf_intervention::services::session::SessionService::new(db.clone()));
    let two_factor_service = Arc::new(rpma_ppf_intervention::services::two_factor::TwoFactorService::new(db.clone()));
    let client_service = Arc::new(rpma_ppf_intervention::services::ClientService::new(db.clone()));
    let task_service = Arc::new(rpma_ppf_intervention::services::TaskService::new(db.clone()));
    let task_import_service = Arc::new(rpma_ppf_intervention::services::task_import::TaskImportService::new(db.clone()));
    let dashboard_service = Arc::new(rpma_ppf_intervention::services::DashboardService::new(db.clone()));
    let intervention_service = Arc::new(rpma_ppf_intervention::services::InterventionService::new(db.clone()));
    let material_service = Arc::new(rpma_ppf_intervention::services::MaterialService::new(db.clone()));
    let photo_service = Arc::new(rpma_ppf_intervention::services::PhotoService::new(db.clone()));
    let analytics_service = Arc::new(rpma_ppf_intervention::services::AnalyticsService::new(db.clone()));
    let settings_service = Arc::new(rpma_ppf_intervention::services::SettingsService::new(db.clone()));
    let cache_service = Arc::new(rpma_ppf_intervention::services::cache::CacheService::new(100).unwrap());
    let report_job_service = Arc::new(rpma_ppf_intervention::services::report_jobs::ReportJobService::new(db.clone()));
    let performance_monitor_service = Arc::new(rpma_ppf_intervention::services::performance_monitor::PerformanceMonitorService::new());
    let command_performance_tracker = Arc::new(rpma_ppf_intervention::services::performance_monitor::CommandPerformanceTracker::new());
    let prediction_service = Arc::new(rpma_ppf_intervention::services::prediction::PredictionService::new(db.clone()));
    let sync_queue = std::sync::Arc::new(rpma_ppf_intervention::sync::SyncQueue::new(100));
    let background_sync = std::sync::Arc::new(std::sync::Mutex::new(rpma_ppf_intervention::sync::BackgroundSyncService::new(db.clone())));
    let event_bus = std::sync::Arc::new(rpma_ppf_intervention::services::event_bus::InMemoryEventBus::new());

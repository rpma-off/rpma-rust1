...
ALTER TABLE intervention_steps ADD COLUMN IF NOT EXISTS location_lat REAL
    CHECK(location_lat IS NULL OR (location_lat >= -90 AND location_lat <= 90));
ALTER TABLE intervention_steps ADD COLUMN IF NOT EXISTS location_lon REAL
    CHECK(location_lon IS NULL OR (location_lon >= -180 AND location_lon <= 180));
ALTER TABLE intervention_steps ADD COLUMN IF NOT EXISTS location_accuracy REAL;
